...
class AggressiveBot(Bot, BombSpawnable):
    ...
    def think(self):
        ...
        # If there are no available bonuses, but can safely spawn bomb to break obstacles, it will do that.
        ...
        if self.moving == 0:
            ...
            if min_dist < float('inf'): # there is player that can be reached
                ...
                for bonus in get_bonuses(globals.entities):
                    x, y = bonus.x, bonus.y
                    if not in_valid_range(x, y, globals.cols, globals.rows):
                        continue
                ...
                if min_weighted_dist == -1: # there is bonus and path to it isn't risky:
                    for bonus in get_bonuses(globals.entities):
                        x, y = bonus.x, bonus.y
                        if not in_valid_range(x, y, globals.cols, globals.rows):
                            continue
                    ...
                    for bonus in get_bonuses(globals.entities):
                        ...
                        if not in_valid_range(x, y, globals.cols, globals.rows):
                            continue
                        if self.used[x][y] and self.weighted_dist[x][y] == min_weighted_dist and self.dist[x][y] == min_dist:
                            destinations.append((x, y))

                    print("ooo", self.x, self.y, destinations)
                    ...
                    if max_dist > self.bomb_power: # safe bomb spawning
